...
app.service('itunesService', function($http, $q){
	...
	this.sMethod = function(name) {

		artist = name
		var dfr = $q.defer();
		$http({
			method: 'JSONP',
			url: 'https://itunes.apple.com/search?term=' + artist + '&callback=JSON_CALLBACK&attribute=artistTerm&media=music&explicit=yes&limit=100',
			}).then(function(response){
				console.log(response)
				dfr.resolve(response.data.results)
			})
		return dfr.promise;
	}
